...
function IconButton({ children, label, onClick, badge, active }: { children: ReactNode; label: string; onClick: () => void; badge?: number; active?: boolean }) {
  return (
    <button
      ...
      onClick={onClick}
      className={
        'relative flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-[10px] border border-[color:var(--border)] transition hover:bg-[color:var(--hover)] ' +
        (active ? 'bg-[color:var(--surface-2)] text-[color:var(--ink)]' : 'text-[color:var(--ink-dim)]')
      }
    >
      {children}
      {badge ? (
        <span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[color:var(--accent)] px-1 text-[0.6rem] font-bold text-[color:var(--accent-ink)]">
          {badge}
        </span>
      ) : null}
    </button>
