...
Super Admins see the **Admin** tab: people and roles (including **removing accounts** — this calls `/api/admin-delete-user`, which needs `SUPABASE_SERVICE_ROLE_KEY` on Vercel), groups, the **Discover events** catalogue (public events anyone can subscribe to), compliance policy, analytics and the audit log. Whoever creates a group is its Group Admin by default. In local demo mode (no Supabase) everyone is treated as Super Admin so the console is explorable.
